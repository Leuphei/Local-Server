...
public class RangeMaxHit extends RangeData {
	...
	public static int calculateRangeAttack(Player c) {
		...
		/*if (c.TwistedBow()) {
			rangeLevel += c.getLevelForXP(c.playerXP[c.playerRanged]) * 0.16;
		}*/
		if (c.prayerActive[3]) {
			rangeLevel *= 1.05;
	...
	public static boolean wearingTwistedBow(Player c) {
		return c.playerEquipment[3] != -1 && Item.getItemName(c.playerEquipment[3]).toLowerCase().contains("twisted bow");
	}
	...
	public static int maxHit(Player c) {
		...
		int rangedStrength = 
			...
			//	wearingTwistedBow(c) ? getRangeStr(c.playerEquipment[c.playerWeapon]) :
