	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		case 29777:
			...
			for (String username : c.clan.activeMembers) {
				...
				if (p == null || p.disconnected || !p.isActive || p.getName().equalsIgnoreCase(c.getName())) {
					continue;
				}
		...
		case 29778:
			c.sendMessage("You have ended raids");
			break;
		...
		case 29241:
			if (!c.getRights().isOrInherits(Right.DONATOR) && !c.getRights().isOrInherits(Right.ADMINISTRATOR)) {
				c.sendMessage("<img=6> Sorry, you need to be a donator to use this feature.");
				return;
			}
