...
/**
* Teleport Handler Class - Used for our new teleporting system
* Probably should have used enums, oh well..
* @author Tyler
*/
public class TeleportHandler {
	...
	String[] monsterNames = {"Select your Monster Teleport", "Rock Crabs", "Cows", "Yaks", "Bob's Island", "Desert Bandits", "Elf Warriors", "Dagannoths", "Chickens", "Slayer Tower", "Brimhaven Dungeon", "Taverley Dungeon", "Stronghold Cave", "Relleka Dungeon", "Mithril Dragons", "Asgarnian Ice Cave", "Catacombs", "Cave Kraken", "Smoke Devils", "", ""};
	String[] minigameNames = {"Select your Minigame Teleport", "Raids", "Warriors Guild", "Pest Control", "Fight Caves", "Barrows", "Clan Wars", "Shayzien Assault", "Mage Arena", "Duel Arena", "", "", "", "", "", "", "", "", "", "", ""};
	String[] bossNames = {"Select your Boss Teleport", "Barrelchest", "Dagannoth Kings", "King Black Dragon@red@(Wild)", "Giant Mole@red@(Deep Wild)", "Kalphite Queen", "God Wars Dungeon", "Corporeal Beast", "Dagannoth Mother", "Kraken", "Zulrah", "Cerberus", "Smoke Devil", "Abyssal Sire", "Demonic Gorillas", "Lizardman Shaman", "", "", "", "", ""};	
	String[] wildernessNames = {"Select your Wilderness Teleport", "Green Dragons @red@(10)", "Mage Bank", "Dark Castle @red@(15)", "Hill Giants Multi @red@(18)", "Wildy Agility Course @red@(52)", "Vet'ion @red@(40) ", "Callisto @red@(43)", "Scorpia @red@(54)", "Venenatis @red@(28)", "Chaos Elemental @red@(50)", "Chaos Fanatic @red@(41)", "Crazy Archaeologist @red@(23)", "Skeletal Wyverns @red@(48)", "East Dragons @red@(18)", "Wildy Volcano @red@(53)", "Chaos Altar @red@(15)", "Lava Dragons @red@(43)", "", "", ""};
	String[] cityNames = {"Select your City Teleport", "Varrock", "Yanille", "Edgeville", "Lumbridge", "Ardougne", "Neitiznot", "Karamja", "Falador", "Taverly", "Camelot", "Catherby", "Al Kharid", "Morytania", "Shilo Village", "Waterbirth", "Lletya", "Burthorpe", "Entrana", "Draynor", "Trollheim"};
	String[] donatorNames = {"Select your Donator Teleport", "@blu@Donator Lobby", "@blu@Basic Slayer", "@gre@Advanced Slayer", "@gre@Basic Skilling", "@gre@Demons", "@yel@Advanced Skilling", "@yel@Dragons(Hides)", "@yel@Dragons(Metals)", "@yel@Smoke Devils", "@pur@Skeletal Wyverns", "@pur@Fanatic/Archaeologist", "", "", "", "", "", "", "", "", ""};
	String[] otherNames = {"Select your Skilling Teleport", "Slayer Masters", "Lands End", "Skillers Cove", "Woodcutting Guild", "Farming Patches", "Agility - Grace", "Hunter", "Puro Puro", "Mining Guild", "", "", "", "", "", "", "", "", "", "", ""};
	
	public void loadMonsterTab(){
		int[] ids = {65033, 65051, 65053, 65055, 65057, 65059, 65061, 65063, 65065, 65067, 65069, 65071, 65073, 65075, 65077, 65079, 65081, 65083, 65085, 65087, 65089};
		for (int j = 0; j <= 20; j++) {
	...
	public boolean teleportCheck(Player player) {
		/*if (!player.modeTut) {
			player.sendMessage("You must finish the tutorial before teleporting anywhere.");
			return false;
		}*/
		return true;
	}
	...
	public void handleTeleports(Player player, int Id) {
		if (player.inWild() && player.wildLevel > Config.NO_TELEPORT_WILD_LEVEL) {
			//player.sendMessage("You can not open a teleport interface while above level 20 wilderness.");
			return;
		}
		...
		case 51023:
		case 117186:
		//	if (player.getRights().isDonator() || player.getRights().isSuperDonator() || player.getRights().isLegendaryDonator() || player.getRights().isExtremeDonator() || c.getRights().isOrInherits(Right.OWNER)) {
				//player.getPA().showInterface(65000);
				//selection( player, 5);
			//} else {
		...
		if (player.teleSelected == 0) { // Monsters - Rock Crabs
			...
			player.getPA().startTeleport(3301, 5192, 0, "modern"); //change
			...
			if (c.amDonated >= 10 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2038, 4530, 0, "modern");
			...
			player.getPA().startTeleport(3093, 3482, 0, "modern"); //change
		...
		if (player.teleSelected == 0) { // Monster - Cow
			...
			if (c.amDonated >= 10 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2041, 4494, 2, "modern");
				player.sendMessage("Teleporting to "+donatorNames[2]+".");
			} else {
				player.sendMessage("You must be a donator to access this.");
				return;
			}
		...
		if (player.teleSelected == 0) { // Monster - Yaks
			...
			if (c.amDonated >= 50 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2029,4527, 2, "modern");
				player.sendMessage("Teleporting to "+donatorNames[3]+".");
			} else {
				player.sendMessage("You must be a super donator to access this.");
				return;
			}
		} else if (player.teleSelected == 6) { // Other - Skillers Cove
			player.getPA().startTeleport(1721,3464, 0, "modern");
		...
		if (player.teleSelected == 0) { // Monsters - Bob's Island
			...
			player.getPA().startTeleport(3078, 3924, 0, "modern");
			player.sendMessage("Teleporting to "+bossNames[4]+".");
			//player.sendMessage("Right click and Look-inside mole hills to fight the Giant Mole!");
			...
			if (c.amDonated >= 50 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2029,4528, 3, "modern");
				player.sendMessage("Teleporting to "+donatorNames[4]+".");
			} else {
				player.sendMessage("You must be a super donator to access this.");
				return;
			}
		...
		if (player.teleSelected == 0) { // Monsters - Desert Bandit
			...
			if (c.amDonated >= 50 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(1994,4530, 3, "modern");
				player.sendMessage("Teleporting to "+donatorNames[5]+".");
			} else {
				player.sendMessage("You must be a super donator to access this.");
				return;
			}
		...
		if (player.teleSelected == 0) { // Monsters - Elf Warrior
			...
			if (c.amDonated >= 150 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(1994,4530, 1, "modern");
				player.sendMessage("Teleporting to "+donatorNames[6]+".");
			} else {
				player.sendMessage("You must be an extreme donator to access this.");
				return;
			}
		} else if (player.teleSelected == 6) { // Other - Agility - Grace
			player.getPA().startTeleport(3094,3485, 0, "modern");
		...
		if (player.teleSelected == 0) { // Monsters - Dagannoths
			...
			if (c.amDonated >= 150 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2044,4494, 0, "modern");
				player.sendMessage("Teleporting to "+donatorNames[7]+".");
			} else {
				player.sendMessage("You must be an extreme donator to access this.");
				return;
			}
		...
		if (player.teleSelected == 0) { // Monsters - Chickens
			...
			if (c.amDonated >= 150 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2044,4494, 1, "modern");
				player.sendMessage("Teleporting to "+donatorNames[8]+".");
			} else {
				player.sendMessage("You must be an extreme donator to access this.");
				return;
			}
		...
		if (player.teleSelected == 0) { // Monsters - Slayer Tower
			...
			if (c.amDonated >= 150 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2036,4530, 1, "modern");
				player.sendMessage("Teleporting to "+donatorNames[9]+".");
			} else {
				player.sendMessage("You must be an extreme donator to access this.");
				return;
			}
		} else if (player.teleSelected == 6) { // Other - Mining Guild
			player.getPA().startTeleport(3046,9756, 0, "modern");
		...
		if (player.teleSelected == 0) { // Monsters - Brimhaven Dungeon
			...
			if (c.amDonated >= 300 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(1996,4530, 2, "modern");
				player.sendMessage("Teleporting to "+donatorNames[10]+".");
			} else {
				player.sendMessage("You must be a legendary donator to access this.");
				return;
			}
		} else {
			player.sendMessage("Suggest Something on the forums for this spot!");
		...
		if (player.teleSelected == 0) { // Monsters - Taverley Dungeon
			...
			if (c.amDonated >= 300 || c.getRights().isOrInherits(Right.OWNER)) {
				player.getPA().startTeleport(2044,4493, 3, "modern");
				player.sendMessage("Teleporting to "+donatorNames[11]+".");
			} else {
				player.sendMessage("You must be a legendary donator to access this.");
				return;
			}
		...
		}/* else if (c.getRights().isOrInherits(Right.VIP) || c.getRights().isOrInherits(Right.OWNER)) {
			if (player.teleSelected == 5) { // Legendary - Runecrafting
				player.getPA().showInterface(26100);
		...
		if (player.teleSelected == 0) { // Monsters - Relleka Dungeon
			...
			player.getPA().startTeleport(3432,3451, 0, "modern");
