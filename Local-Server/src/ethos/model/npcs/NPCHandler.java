...
import ethos.model.minigames.inferno.JalTokHealers;
	...
	public boolean isAggressive(int i, boolean searching) {
		if(!searching){
			if (Boundary.isIn(npcs[i], Boundary.GODWARS_BOSSROOMS) 
				...
				|| Boundary.isIn(npcs[i], Boundary.Jad3))
...
//				}
				...
				if (npcs[i].isDead) {
					...
					if (npcs[i].actionTimer == 0 &&!npcs[i].applyDead&&!npcs[i].needRespawn) {
						...
						case 7605: //Skeletal mystic
						case 7606: //Skeletal mystic
							SkeletalMystic.deathCount++;
							SkeletalMystic.rewardPlayers(player);
							break;
						case 7544://Tekton
							Tekton.rewardPlayers(player);
							Tekton.specialAmount = 0;
							break;
						case 5129:
						...
						case 3127:
							...
							break;
						/*case 5762: //Not in use
						case 5744:
							player.raidPoints += 1;
							player.sendMessage("You currently have @red@" + player.raidPoints + "@bla@ Assault Points.");
							break;
						case 7595:
							player.raidPoints += 2;
							player.sendMessage("You currently have @red@" + player.raidPoints + "@bla@ Assault Points.");
...
//						break;
				...
				if (npcs[i].npcType == 7705 || npcs[i].npcType == 7704) {
					int distanceFromTarget = c.distanceToPoint(npcs[i].getX(), npcs[i].getY());

					List<NPC> Jadhealer = Arrays.asList(NPCHandler.npcs);
					
					if (distanceFromTarget > 0 && JalTokHealers.stage <= 0 && Jadhealer.stream().filter(Objects::nonNull).anyMatch(n -> n.npcType == 7705 && !n.isDead && n.getHealth().getAmount() > 0)) {
						NPC JALJOK_JAD1 = NPCHandler.getNpc(7704);
						Damage heal = new Damage(damage + Misc.random(40 + 5) + (secondDamage > 0 ? secondDamage : 0));
						if (JALJOK_JAD1 != null && JALJOK_JAD1.getHealth().getAmount() < 150) {
							JALJOK_JAD1.getHealth().increase(heal.getAmount());
						}
					}
				}
	...
	public void loadSpell(Player player, int i) {
		...
		case 7700: //NPC ID
			...
			if (random == 0) {
				npcs[i].attackType = CombatType.MAGE;
				npcs[i].projectileId = 448; // Projectile GFX
				npcs[i].endGfx = 157;
				npcs[i].hitDelayTimer = 5;
				npcs[i].attackTimer = 8;
			} else if (random == 1) {
		...
		case 7706: //NPC ID
			...
			if (random == 0) {
				npcs[i].attackType = CombatType.MAGE;
				npcs[i].projectileId = 1375; // Projectile GFX
				npcs[i].endGfx = -1;
				npcs[i].attackTimer = 14;
				npcs[i].hitDelayTimer = 6;
			...
			if (ignoresProjectile(i)) {
				...
				if (c.respawnTimer <= 0) {
					...
					if (npcs[i].projectileId > 0) {
						...
						int centerY = nY + npcs[i].getSize() / 2;
						c.getPA().createPlayersProjectile(centerX, centerY, offX, offY, 50, getProjectileSpeed(i), npcs[i].projectileId,
								getProjectileStartHeight(npcs[i].npcType, npcs[i].projectileId), getProjectileEndHeight(npcs[i].npcType, npcs[i].projectileId), -c.getIndex() - 1,
