	...
	public static void receive(Player player, NPC npc) {
		...
		pet.ifPresent(p -> {
			...
			if (RandomUtils.nextInt(0, p.droprate) == 1) {
				...
				PlayerHandler.executeGlobalMessage("[@red@PET@bla@] @cr20@<col=255> <img="+rights+">" + Misc.formatPlayerName(player.playerName) + "</col> received a pet from <col=255>" + p.parent + "</col>.");
			}
		});
