...
public class PlayerAssistant {
	...
	private void playerWalk(int x, int y) {
	...
	public void loadQuests() {
		...
		sendFrame126(" @or1@Pyongyang", 29155);
		...
		sendFrame126("@or2@<img=14> Pyongyan Channel: @or1@Click Here", 29178);
		sendFrame126("@or2@<img=11> Pyongyang Discord: @or1@Click Here", 29179);
		sendFrame126("@or2@<img=19> Pyongyang Store: @or1@Click Here", 29180);
...
public void sendFrame107() {
	...
	public void giveLife() {
		...
		} else if (Boundary.isIn(c, Boundary.RAID_MAIN)) {
		...
		} else if (Boundary.isIn(c, Boundary.RAID_MAIN) && c.getRaids().getRaidInstance() != null) {
			c.getRaids().teleportToStart();
	...
	public void levelUp(int skill) {
		...
		if (c.maxRequirements(c)) {
			PlayerHandler.executeGlobalMessage("<img=10></img>[<col=255>News</col>] <col=CC0000>" + Misc.capitalize(c.playerName) + "</col> has <col=255>MAXED</col> out all skills, congratulations.");
		}
		if (getLevelForXP(c.playerXP[skill]) == 99) {
			Skill s = Skill.forId(skill);
			PlayerHandler.executeGlobalMessage("<img=10></img>[<col=255>News</col>] <col=CC0000>" + Misc.capitalize(c.playerName) + "</col> has reached level 99 <col=CC0000>"
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
		...
		if (oldExperience < 200_000_000 && oldExperience + amount >= 200_000_000) {
			Skill s = Skill.forId(skill);
			PlayerHandler.executeGlobalMessage("<img=10></img>[<col=255>News</col>] <col=CC0000>" + Misc.capitalize(c.playerName) + "</col> has reached 200M XP in <col=CC0000>"
					+ s.toString() + "</col>, congratulations.");
		}
	...
	public void addJadgear() {
			c.getItems().wearItem(4753, 1, c.playerHat);
			c.getItems().wearItem(19547, 1, c.playerAmulet);
			c.getItems().wearItem(11828, 1, c.playerChest);
			c.getItems().wearItem(11830, 1, c.playerLegs);
			c.getItems().wearItem(13237, 1, c.playerFeet);
			c.getItems().wearItem(7462, 1, c.playerHands);
			c.getItems().wearItem(19710, 1, c.playerRing);
			c.getItems().wearItem(20997, 1, c.playerWeapon);
			c.getItems().wearItem(13280, 1, c.playerCape);
			c.getItems().wearItem(11212, 1000, c.playerArrows);
	
			c.getItems().addItem(3024, 4);
			c.getItems().addItem(6685, 4);
			c.getItems().addItem(2444, 4);
			c.sendMessage("");
		}
