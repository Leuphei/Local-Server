...
import ethos.model.minigames.inferno.Jad3;
...
public Inferno inferno = new Inferno(this, Boundary.INFERNO, 0);
	...
	public void highscores() {
		getPA().sendFrame126("Pyongyang - Top PKers Online", 6399); // Title
		for (int i = 0; i < 10; i++) {
			if (ranks[i] > 0) {
	...
	public void destruct() {
		...
		if (jad3 != null) {
			InstancedAreaManager.getSingleton().disposeOf(jad3);
		}
		if (skotizo != null) {
	...
	public void initialize() {
		try {
			...
			sendMessage("<img=16><trans=170>Welcome to <col=255>Pyongyang</col></trans>, " + nameIdentifier + "<col=" + rights.getPrimary().getColor() + ">" + playerName+ "</col>.");
			sendMessage("<img=16>@red@Announcement@bla@: Make sure your client is the latest! v1.10.1");
...
//			getPA().walkableInterface(42300);
	...
	public Jad3 createJad3Instance() {
		Boundary boundary = Boundary.INFERNO;
		
		int height = InstancedAreaManager.getSingleton().getNextOpenHeightCust(boundary, 4);
		
		jad3 = new Jad3(this, boundary, height);
...
public boolean TwistedBow() {
	if (getItems().isWearingItem(20997))
			return true;
	return AttackEventRunning;
}
