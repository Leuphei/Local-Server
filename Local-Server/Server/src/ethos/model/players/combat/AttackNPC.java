...
//				break;
			...
			case 7704:
				...
				if (JalTokHealers.stage > 0 && Jadhealer.stream().filter(Objects::nonNull).anyMatch(n -> n.npcType == 7705 && !n.isDead && n.getHealth().getAmount() > 0)) {
					NPC JALJOK_JAD1 = NPCHandler.getNpc(7704);
					Damage heal = new Damage(Misc.random(40 + 5));
			...
			if (damage.getCombatType() != null) {
				...
				case RANGE:
					...
					if (Server.npcHandler.getNPCs()[i].attackTimer > 3) {
						if (npc.npcType != 2042 && npc.npcType != 2043 & npc.npcType != 2044 && npc.npcType != 3127 && npc.npcType != 7700 && npc.npcType != 7704 && npc.npcType != 319) {
				...
				case MAGE:
					...
					if (c.getCombat().getEndGfxHeight() == 100 && damage.getAmount() > 0) { // end
						...
						if (Server.npcHandler.getNPCs()[i].attackTimer > 3) {
							if (npc.npcType != 2042 && npc.npcType != 2043 & npc.npcType != 2044 && npc.npcType != 3127 && npc.npcType != 7700 && npc.npcType != 7704 && npc.npcType != 7413) {
								NPCHandler.startAnimation(c.getCombat().npcDefenceAnim(i), i);
							}
						}
			...
			if (c.attackTimer <= 0) {
				...
				if (c.getHealth().getAmount() > 0 && !c.isDead && NPCHandler.npcs[i].getHealth().getMaximum() > 0) {
					if (!c.usingMagic) {
						...
						if (Server.npcHandler.getNPCs()[i].attackTimer > 3) {
							if (npcType != 2042 && npcType != 2043 & npcType != 2044 && npcType != 3127 && npcType != 7700 && npcType != 7704 && npcType != 319) {
								NPCHandler.startAnimation(c.getCombat().npcDefenceAnim(i), i);
							}
						}
					} else {
